...
class Nastavenia extends Repository {
    ...
    public function getBillingAdress($id) {
        ...
        $id = $id->fetch();
        $result = $this->db->query('
        SELECT billing_address.id_billing_address, billing_address.surename, billing_address.name, billing_address.city, billing_address.street, billing_address.postcode,billing_address.phone, country.name as country_name, country.id_country as id_country FROM billing_address
    ...
    public function getShippingAdress($id) {
        $id = $this->db->query('SELECT default_shipping_address_id FROM users WHERE id = ' . $id . '');
        $id = $id->fetch();
        $result = $this->db->query('
        SELECT delivery_address.id_delivery_address, delivery_address.surename, delivery_address.name, delivery_address.city, delivery_address.street, delivery_address.postcode,delivery_address.phone, country.name as country_name, country.id_country as id_country FROM delivery_address
        INNER JOIN country ON delivery_address.country=country.id_country 
        WHERE delivery_address.id_delivery_address = ' . $id->default_shipping_address_id . '
        ');
        $rows = $result->fetch();
        return $rows;
    }
